...
func list{{.ProductName}}(_ *cobra.Command, _ []string) {
	...
	if err != nil {
		fmt.Printf("error fetching {{.Path}}: %s\n", err)
		return
	}
	...
	internal.RenderTable(unmarshalled, {{.ProductNameLower}}ColumnsToDisplay)
}
...
func get{{.ProductName}}(_ *cobra.Command, args []string) {
	...
	if err := client.Get(fmt.Sprintf("{{.Path}}/%s", url.PathEscape(args[0])), &object); err != nil {
		fmt.Printf("error fetching {{.ProductName}}: %s\n", err)
		return
	}

	internal.RenderObject(object, {{.ProductNameLower}}ColumnsToDisplay[0])
}
