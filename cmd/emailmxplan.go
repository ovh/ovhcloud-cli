...
func listEmailMXPlan(_ *cobra.Command, _ []string) {
	...
	if err != nil {
		log.Fatalf("error fetching /email/mxplan: %s\n", err)
	}
	...
	internal.OutputTable(unmarshalled, emailmxplanColumnsToDisplay, jsonOutput, yamlOutput)
}
...
func getEmailMXPlan(_ *cobra.Command, args []string) {
	...
	if err := client.Get(fmt.Sprintf("/email/mxplan/%s", url.PathEscape(args[0])), &object); err != nil {
		log.Fatalf("error fetching EmailMXPlan: %s\n", err)
	}

	internal.OutputObject(object, emailmxplanColumnsToDisplay[0], jsonOutput, yamlOutput)
}
