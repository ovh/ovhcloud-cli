...
// and returns only regions that have the share API available.
...
// fetchFileShareNetworks fetches available private networks for file share creation
func (m Model) fetchFileShareNetworks() tea.Cmd {
	...
	return func() tea.Msg {
		...
		// Pre-extract the OpenStack UUID for our target region into a synthetic field
		// so the UI doesn't need to traverse the regions array.
		for i, net := range networks {
			...
			// Only include networks that are active in the target region
			if openstackId != "" {
				networks[i]["_openstackId"] = openstackId
			}
		}
		// Filter to networks that have the region active
		var filtered []map[string]interface{}
...
// fetchFileShareSubnets fetches available subnets for a specific private network
...
// createFileShare creates a new NFS file share
...
// fetchFileStorageData fetches the list of NFS file shares across all supported regions
func (m Model) fetchFileStorageData() dataLoadedMsg {
	...
	for _, name := range regionNames {
		go func(regionName string) {
			...
			// Tag each share with its region
			for i := range shares {
...
// createFileStorageTable creates a table rendering file storage shares
...
// ─── File Storage wizard message handlers ────────────────────────────────────
