...
func (m Model) dbFilteredFlavors() []string {
	...
	if len(flavors) == 0 {
		// Fallback: filter capabilities flavors by prefix matching engine category.
		// Operational engines (postgresql, mysql, mongodb, valkey) use b3-* flavors.
		// Analytics engines (kafka, cassandra, etc.) use db2-* flavors.
		prefix := "b3-"
		if strings.EqualFold(m.wizard.dbEngineCategory, "analysis") {
			prefix = "db2-"
		}
		...
		// Widen search if still empty
		if len(flavors) == 0 {
			for _, f := range m.wizard.dbFlavors {
				name := getStringValue(f, "name", "")
				if !strings.HasPrefix(name, "db2-") {
					flavors = append(flavors, name)
				}
			}
		}
...
func (m Model) dbNodesConstraints() (int, int) {
	...
	min := 1
	max := 1
	if v, ok := avail["minNodeNumber"].(float64); ok {
		min = int(v)
	}
	if v, ok := avail["maxNodeNumber"].(float64); ok {
		max = int(v)
	}
	if min < 1 {
		min = 1
...
func (m Model) dbStorageConstraints() (int, int, int) {
	...
	if avail != nil {
		if v, ok := avail["minDiskSize"].(float64); ok && v > 0 {
			min = int(v)
		}
		if v, ok := avail["maxDiskSize"].(float64); ok && v > 0 {
			max = int(v)
		}
		if v, ok := avail["stepDiskSize"].(float64); ok && v > 0 {
			step = int(v)
		}
	}
	return min, max, step
