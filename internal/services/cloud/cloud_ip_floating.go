...
var (
	...
	//go:embed templates/cloud_ip_floating.tmpl
	cloudFloatingIPTemplate string
)

// fetchFloatingIPs fetches all floating IPs for the given project, optionally
// filtered by CloudFloatingIPRegionFilter.
func fetchFloatingIPs(projectID string) ([]map[string]any, error) {
	var regions []any
	...
	if err != nil {
		return nil, fmt.Errorf("failed to fetch floating IPs: %w", err)
	}
...
func ListFloatingIPs(_ *cobra.Command, _ []string) {
	...
	allFloatingIPs, err := fetchFloatingIPs(projectID)
	if err != nil {
		display.OutputError(&flags.OutputFormatConfig, "%s", err)
		return
	}
